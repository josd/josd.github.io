...
'http://josd.github.io/helgoland#equation'([A,B],C) :-
    findall(E,
...
% query
query('http://josd.github.io/helgoland#equation'([x^2-3*x+2=0,x],_ANSWER)).
query('http://josd.github.io/helgoland#equation'([cos(x)*(1-2*sin(x))=0,x],_ANSWER)).
query('http://josd.github.io/helgoland#equation'([2^(2*x)-5*2^(x+1)+16=0,x],_ANSWER)).
