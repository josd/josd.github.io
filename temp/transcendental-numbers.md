...
Most of the numbers we first learn to treat as *exact*—fractions like \(3/7\), roots like \(\sqrt{2}\), and even the golden ratio—share a comforting property: you can describe them as solutions to polynomial equations.

- \(\sqrt{2}\) is the positive solution of \(x^2-2=0\).
- The golden ratio \(\varphi\) solves \(x^2-x-1=0\).
...
And yet, proving transcendence for a particular “everyday” constant can be maddeningly difficult. For example, despite overwhelming intuition about how “complicated” these should be, we still do not know whether numbers like \(e+\pi\) or \(e\pi\) are even irrational, let alone transcendental.
...
\]
The 1s appear at positions \(1!,2!,3!,\dots\), creating approximations that are “too good to be algebraic.”

This was the first time mathematicians could point to a specific, explicit transcendental number and prove it.

---

## The headline results: \(e\), \(\pi\), and the end of “squaring the circle”
...
Lindemann’s result famously resolves an ancient geometric puzzle: you cannot “square the circle” using only straightedge and compass. The reason is that such a construction would force \(\pi\) to be algebraic—contradicting Lindemann’s theorem.
...
A useful takeaway from the Lindemann–Weierstrass theorem is:

> If \(\alpha\neq 0\) is algebraic, then \(e^{\alpha}\) is transcendental.

This doesn’t answer everything—\(\pi\) itself isn’t of the form \(e^\alpha\)—but it sets the tone: the exponential function tends to push algebraic inputs out into transcendental territory.

### 2) Gelfond–Schneider (Hilbert’s 7th problem)
The **Gelfond–Schneider theorem** (1934) goes further and is famous for settling Hilbert’s 7th problem. In a friendly form:

> If \(a\) is algebraic with \(a\notin\{0,1\}\), and \(b\) is an algebraic number that is *not rational*, then every value of \(a^b\) is transcendental.

This instantly proves the transcendence of several “nice-looking” constants, such as:
- \(2^{\sqrt{2}}\)
- \(e^{\pi}\) (more on the trick behind this below)
- \(i^i\)
...
i^i = e^{-\pi/2},
\]
a perfectly ordinary positive real number (about \(0.2079\ldots\)).
...
|---|---|---|
| \(e+\pi\) | **Unknown** | Not known to be irrational; not known to be transcendental. |
| \(e\cdot\pi\) | **Unknown** | Not known to be irrational; not known to be transcendental. |
| \(e^{\pi}\) | **Transcendental** | Proven using Gelfond–Schneider (via the identity \(e^{\pi}=(-1)^{-i}\)). |
| \(\pi^{e}\) | **Unknown** | Not known to be irrational; not known to be transcendental. |
| \(i^{i}\) (`i**i`) | **Transcendental** | One value equals \(e^{-\pi/2}\); transcendence follows from Gelfond–Schneider. |

A neat “in-between” fact you can prove with high-school algebra (plus one key field-theory idea) is:

> At least one of \(e+\pi\) and \(e\pi\) must be transcendental.

Why? If both \(e+\pi\) and \(e\pi\) were algebraic, then \(e\) and \(\pi\) would satisfy the quadratic
\[
x^2-(e+\pi)x+e\pi=0,
\]
forcing \(e\) and \(\pi\) to be algebraic—contradiction. The argument doesn’t tell us *which* of the two is transcendental, but it shows that “both algebraic” is impossible.
...
\]
are famous; at odd integers \(s=3,5,7,\dots\) they are especially mysterious. A lot of modern work builds “motivic” versions of these constants to predict the web of relations among them. Even when transcendence is out of reach, mathematicians can often prove weaker but meaningful facts—like irrationality or linear independence in large families.
...
- A survey-level overview of periods and transcendence: work of Michel Waldschmidt.
- The original viewpoint on periods: Kontsevich–Zagier, *Periods*.
- An introduction to functional transcendence via o-minimality: notes by Jonathan Pila.
- Work around irrationality of odd zeta values: Ball–Rivoal and later refinements.
