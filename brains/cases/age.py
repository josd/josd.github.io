...
There is no external reasoner. All reasoning is implemented in this file:
  - unification over triples with variables,
  - one rule for :ageAbove,
  - and three built-ins.


Two people in the KB
--------------------
We use a fixed “today”:
...
and two people:
  ...
  :josD  :birthDay  "1956-01-10" .

As of 2026-01-31, these roughly satisfy:

  age(:patH) ≈ 81.45 years
  age(:josD) ≈ 70.06 years

So the rule entails:

  :patH :ageAbove "P80Y"^^xsd:duration .
  :patH :ageAbove "P70Y"^^xsd:duration .
  :josD :ageAbove "P70Y"^^xsd:duration .

but **not**:

  :josD :ageAbove "P80Y"^^xsd:duration .
...
it performs two queries:
  ...
  1. (?S, ":ageAbove", "P80Y")
     • shows that only :patH satisfies the 80-year threshold.

  2. (?S, ":ageAbove", "P70Y")
     • shows that both :patH and :josD satisfy the 70-year threshold,
       illustrating that `holds2` cleanly produces multiple answers.

For these it prints three ARC-style sections:

  1. Answer
     • the subjects satisfying :ageAbove "P80Y" and :ageAbove "P70Y",
       plus their approximate numeric ages.

  2. Reason why
     • the textual rule,
     • a short explanation of how it instantiates,
     • and a step-by-step proof trace for the 80-year query.

  3. Check (harness)
     • at least five independent tests checking:
         - that :ageAbove "P80Y" holds only for :patH,
         - that :ageAbove "P100Y" holds for nobody,
         - that :ageAbove "P70Y" holds for both :patH and :josD,
...
facts: Set[Triple] = {
    ...
    (":josD", ":birthDay", "1956-01-10"),  # ISO date string
}
...
def duration_to_years(dur: str) -> float:
    """Example: 'P80Y' -> 80.0 (only integer years supported)."""
...
def builtin_time_localTime(s, o, θ: Subst) -> Optional[Subst]:
    """'' time:localTime ?D — binds ?D to the fixed 'today' (2026-01-31)."""
    if s != "":
        return None
    today = date(2026, 1, 31)  # fixed “today” for reproducibility
    val = today.isoformat()
    θ2: Subst = dict(θ)
    if is_var(o):
        θ2[o] = val
...
def holds2(
    ...
    if pred == "math:greaterThan":
        ...
        if θ2 is not None:
            rhs = duration_to_years(obj) if isinstance(obj, str) else obj
    ...
    # Ground facts (may yield multiple bindings)
    any_fact = False
...
def solve(query: Triple) -> Tuple[bool, Subst, List[str]]:
    """Return (success?, first_substitution, trace) for a single triple query."""
    trace: List[str] = []
    θ = next(holds2(query, {}, 0, frozenset(), count(1), trace), None)
    return (θ is not None), (θ or {}), trace


def solve_all(query: Triple) -> Tuple[List[Subst], List[str]]:
    """Return (all_substitutions, trace) for a single triple query."""
    trace: List[str] = []
    stepctr = count(1)
    sols = list(holds2(query, {}, 0, frozenset(), stepctr, trace))
    return sols, trace
...
def age_from_birth(birth_str: str) -> float:
    """Convenience: convert a birth date to a numeric age as of 2026-01-31."""
    today = date(2026, 1, 31)
    b = parse_iso_date(birth_str)
    return (today - b).days / 365.2425

def arc_answer() -> None:
    """Print answers for the 80-year and 70-year thresholds."""
    print("Answer")
    print("------")

    # 1. P80Y
    q80: Triple = ("?S", ":ageAbove", "P80Y")
    ok80, θ80, _ = solve(q80)
    if ok80:
        s80 = θ80["?S"]
        birth80 = next(o for (subj, p, o) in facts if subj == s80 and p == ":birthDay")
        age80 = age_from_birth(birth80)
        print("Query 1: ?S :ageAbove \"P80Y\"^^xsd:duration")
        print(f"  Answer: {s80}")
        print(f"  Approximate age (2026-01-31): ~{age80:.2f} years")
    else:
        print("Query 1: ?S :ageAbove \"P80Y\" — no solution.")
    print()

    # 2. P70Y (may have multiple answers)
    q70: Triple = ("?S", ":ageAbove", "P70Y")
    sols70, _ = solve_all(q70)
    print("Query 2: ?S :ageAbove \"P70Y\"^^xsd:duration")
    if sols70:
        for i, θ in enumerate(sols70, start=1):
            s = θ["?S"]
            birth = next(o for (subj, p, o) in facts if subj == s and p == ":birthDay")
            age_val = age_from_birth(birth)
            print(f"  {i}. {s}  (age ~{age_val:.2f} years)")
    else:
        print("  No solutions.")
    print()
...
def arc_reason(max_lines: int = 40) -> None:
    """Print a short explanation and the proof trace for the P80Y query."""
    ...
    print("} .")
    print()
    print("For the 80-year query we get:")
    if success:
        s = θ["?S"]
        print(f" • ?S  = {s}")
        print(" • ?A  = \"P80Y\" (threshold 80 years)")
    else:
        print(" • No subject satisfies the rule; but this should not happen in this KB.")
    print(" • ?B  = birthDay(?S)")
    print(" • ?D  = '2026-01-31' (via time:localTime)")
    print(" • ?F  = years(?D - ?B) (via math:difference)")
    print(" • constraint: ?F > 80 (via math:greaterThan)")
    print()

    print("Proof trace (for the first solution):")
...
def run_checks() -> List[str]:
    ...
    Run several independent tests to sanity-check the tiny engine:
      ...
      1) :ageAbove P80Y succeeds for :patH and not for :josD.
      2) :ageAbove P100Y fails.
      3) :ageAbove P70Y succeeds for both :patH and :josD.
      4) math:difference gives a plausible age for :patH in (80, 81) years.
      5) math:greaterThan behaves correctly around the P80Y/P81Y thresholds.
      6) Unification for the rule head works as expected.
    """
    notes: List[str] = []

    # 1) P80Y succeeds only for :patH
    ok80, θ80, _ = solve(("?S", ":ageAbove", "P80Y"))
    check(ok80, "Expected some subject to satisfy :ageAbove P80Y.")
    s80 = θ80.get("?S")
    check(s80 == ":patH", f"Expected :patH for P80Y, got {s80!r}.")
    notes.append("PASS 1: only :patH satisfies :ageAbove 'P80Y'^^xsd:duration.")
    ...
    # 3) P70Y succeeds for both :patH and :josD
    sols70, _ = solve_all(("?S", ":ageAbove", "P70Y"))
    who70 = {θ["?S"] for θ in sols70}
    check({":patH", ":josD"} == who70, f"Expected {{:patH, :josD}} for P70Y, got {who70}.")
    notes.append("PASS 3: both :patH and :josD satisfy :ageAbove 'P70Y'^^xsd:duration.")

    # 4) math:difference sanity: :patH age is between 81 and 82 years
    θ_diff = builtin_math_difference(("2026-01-31", "1944-08-21"), "?F", {})
    check(θ_diff is not None, "math:difference should succeed for valid dates.")
    age_pat = θ_diff["?F"]
    check(isinstance(age_pat, float), "math:difference result should be a float.")
    check(81.0 < age_pat < 82.0,
          f":patH's age should be in (81, 82) years, got {age_pat:.6f}.")
    notes.append("PASS 4: math:difference returns :patH age between 81 and 82 years.")

    # 5) math:greaterThan around P80Y/P82Y thresholds for :patH
    check(
        builtin_math_greaterThan(age_pat, "P80Y", {}) is not None,
        ":patH age should be greater than 'P80Y'.",
    )
    check(
        builtin_math_greaterThan(age_pat, "P82Y", {}) is None,
        ":patH age should NOT be greater than 'P82Y'.",
    )
    notes.append("PASS 5: math:greaterThan behaves correctly around 'P80Y'/'P82Y' for :patH.")
    ...
    check(
        θ_unif.get("?S") == ":patH" and θ_unif.get("?A") == "P80Y",
